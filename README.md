...
## 파일 내용
- Conclusion : 최종 결론을 도출하는 코드가 들어있습니다.
    - conclusion.r : 최종 결론을 도출하는 코드입니다.
- Family : 가계자금 및 금융자산에 대해 분석한 코드들이 들어있습니다.
    - CA.r : 군집분석에 대한 코드가 들어있습니다.
    - EDA.r : 데이터를 탐색하는 코드가 들어있습니다.
    - FA.r : 인자 분석에 대한 코드가 들어있습니다.
    - PCA.r : 주성분 분석에 대한 코드가 들어있습니다.
- Investment : 투자 전략에 대해 분석한 코드가 들어있습니다.
    - CA.r : 군집분석에 대한 코드가 들어있습니다.
    - EDA.r : 데이터를 탐색하는 코드가 들어있습니다.
    - FA.r : 인자 분석에 대한 코드가 들어있습니다.
    - PCA.r : 주성분 분석에 대한 코드가 들어있습니다.
...
## 설명
- 해당 코드에 대한 설명은 각 폴더 별 readme에 업로드 하였습니다!
- 각각의 폴더에 들어가서 readme를 확인해보세요!
...
## 결론
### 1. 들어가기전에
지역 및 연령별 가계자금 및 금융자산 데이터를 활용한 분석 통해 표 6.1~6.4에 해당하는 지역 및 연령별 소비와 미래에 대한 정보를 파악할 수 있었고, 동학 개미 운동 신규 고객 투자 정보를 활용하여 지역 및 연령별 주식 상품에 수요 및 투자 여유자금과 안정성과 수익성 사이의 선호도를 파악할 수 있었다. 그리고 두 데이터를 결합하여 각 특징에 맞는 군집을 도출해 냄으로써 소비와 주식투자에 대한 새로운 데이터를 생성하였다. 위와 같은 데이터를 활용한 기대효과는 다음과 같다.

### 2. 각 군집의 특징에 맞는 적절한 주식 상품의 구성 가능성
투자자의 연령 및 지역별 특징에 따라 적절한 주식 상품을 구성하여 판촉 하기 용이해질 것이다. 가령 40대를 겨냥한 주식 상품을 구성한다면, 투자 여유 자산이 많고 안전성보단 수익성을 선호하는 40대 경북과 투자 여유 자산이 적고 안전성을 선호하는 40대 경남은 상품의 구성이 달라질 것이고, 높은 생활수준을 요구하고, 미래 대비가 높은 40대 대전과 이와 정반대인 40대 충남은 주식 상품 판촉을 위한 전략이 달라질 것이다. 즉, 지역 및 연령별 특징을 파악함으로써 상품 판매를 위한 더 효율적이고 매력적인 구성이 가능해질 것이다.

### 3. 역선택 감소의 가능성
위와 같은 통계 데이터를 통해 도출된 결과를 통해 소비자 또한 자신이 속한 집단의 소비성향과 미래에 대한 대비 정도와 주식에 대한 수요 및 여유자금과 안전성에 대한 선호 등의 정보를 획득하고, 이를 개개인의 상황과 비교하며, 이에 알맞게 추천된 주식 상품들을 비교해봄으로써 시장 내에 역선택의 가능성을 감소시킬 수 있을 것이다. 이를 통해 개개인의 상황과 맞지 않게 구성된 주식 상품은 자연스럽게 시장에서 퇴출되고, 효율적인 상품만 남음으로써 시장 내 양화를 구축할 가능성을 열 수 있을 것이다.

### 4. 주식상품 판매 수익 개선의 가능성
1번과 2번의 가능성이 실현된다면, 이에 따라 시장 내에 2가지 효과가 기대된다. 1) 각 군집의 특성에 맞는 주식 상품만 공급하게 되므로 불필요한 주식 상품의 공급이 줄어 비용 절감 2) 제공된 정보를 바탕으로 소비자에 정보의 비대칭이 해소되고, 진입장벽이 낮아져 시장 내 수요 증가. 따라서 위 두 가지를 효과를 고려해본다면, 주식 상품 판매 수익의 개선 가능성이 있다고 볼 수 있다.

### 5. 결론
위와 같은 기대효과가 발생하지 않을 가능성도 분명 존재한다. 위 데이터들은 중윗값을 기준으로 분석했으므로 개개인의 특징과 선호를 정확하게 반영할 수는 없기 때문이다. 하지만, 투자 정보와 가계자금 및 금융자산 데이터를 통해 연령 및 지역별 소비자의 대략적인 특성을 파악해보고, 이를 각각의 군집으로 분류할 수 있음을 보였다는 점에서 의미 있는 데이터라고 볼 수도 있다.
결론적으로 아래와 같이 투자성향에 맞는 상품을 파악할 수 있을 것이다.
