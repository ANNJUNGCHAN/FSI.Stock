...
### CCC & Dindex values & Second differences Dindex values
...
round(ds, 3)
people=rownames(X)
kmeans <- kmeans(Z, 8) # 4 cluster solution
...
### K-means Aggregate with log transformation
aggregate(X, by=list(kmeans$cluster),FUN=mean)

### K-means Aggregate
aggregate(10^(X+1), by=list(kmeans$cluster),FUN=mean)